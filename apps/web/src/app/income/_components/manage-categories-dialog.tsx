...
/**
 * Dialog for managing income categories (D-11).
...
export function ManageCategoriesDialog({ categories }: ManageCategoriesDialogProps) {
  ...
  function handleStartEdit(cat: IncomeCategory) {
    setEditingId(cat.id);
  ...
  function handleRename(id: number) {
    ...
    startTransition(async () => {
      const result = await renameIncomeCategoryAction(id, trimmed);
      if ('error' in result) {
        toast.error(
        ...
        setEditingId(null);
        setEditingName('');
        // Dialog will reflect new name after revalidatePath triggers RSC re-render
      }
    });
  }
